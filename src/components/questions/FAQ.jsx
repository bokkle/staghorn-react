    ...
    <section className="px-4 py-16 md:px-6 lg:px-8 lg:py-28">
      <div className="mx-auto w-full max-w-screen-xl">
        ...
        <div className="flex h-full w-full flex-col divide-y divide-emerald-400 py-6 lg:flex-row lg:gap-4 lg:divide-y-0">
          <div className="w-full divide-y divide-emerald-400 lg:w-1/2">
            {faq
              .filter((_, i, arr) => i <= (arr.length - 1) / 2)
              .map((q, i) => (
                <div
                  key={i}
                  className={`flex cursor-pointer flex-col items-center justify-between ${activeIndex === i && 'bg-emerald-950'}`}
                ...
                >
                  <div className="flex w-full flex-col p-2">
                    <div className="flex items-center justify-between">
                      <div className="py-4">
                        <h4
                          className={`text-xl font-semibold tracking-wide ${activeIndex === i ? 'text-emerald-400' : 'text-zinc-200'}`}
                        >
                          {q.question}
                        </h4>
                      </div>
                    ...
                    <div
                      ...
                      style={{ maxHeight: activeIndex === i ? heights[i] : 0 }}
                    >
                      {q.answer.map((answer, i) => (
                        <p
                          key={i}
                          className="max-w-[95%] text-xl text-zinc-300"
                        >
                          {answer}
                        </p>
                      ))}
          ...
          <div className="w-full divide-y divide-emerald-400 lg:w-1/2">
            {faq
              ...
              .map((q, i) => (
                <div
                  key={i + 7}
                  className={`flex cursor-pointer flex-col items-center justify-between ${activeIndex === i + 7 && 'bg-emerald-950'}`}
